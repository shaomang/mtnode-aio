...
(function (root, factory) {
  ...
  var EN = {
    ...
    "切换到工作流：": "Switched to workflow: ",
    "青柠 Lime": "Lime",
    "请先选择服务商": "Select a provider first",
    "筛选节点标题…": "Filter node titles…",
    "删除当前工作流": "Delete current workflow",
    "输出浏览 · ": "Output · ",
    "思考过程 · ": "Thinking · ",
    "思考内容 · ": "Thoughts · ",
    ...
    "打开工作流保存的文件夹": "Open workflow save folder",
    ...
    "工作流包含无法序列化的数据：": "Workflow contains data that cannot be serialized: ",
    ...
    "工作流已删除，已重建默认工作流": "Workflow deleted; default workflow recreated",
    ...
    "AI 工作流编排：画布式节点编排": "AI workflow orchestration: canvas-based node graph",
    ...
    "导出失败：工作流包含无法序列化的数据": "Export failed: workflow contains data that cannot be serialized",
    ...
    "工作流已设置统一工作目录,本节点只读继承": "Workflow has a shared working directory; this node inherits it read-only",
    "关闭标签（仅从标签条移除，不删除工作流）": "Close tab (remove from the tab bar only; does not delete the workflow)",
    ...
    "导入画布：从 .mtnodes 文件恢复完整工作流": "Import canvas: restore a full workflow from a .mtnodes file",
    ...
    "导出画布：把当前工作流（含图像等资产）打包为 .mtnodes 文件，可迁移到其他电脑": "Export canvas: pack the current workflow (including image assets) into a .mtnodes file, which you can move to another computer",
    ...
    "这是<b>智能会话</b>画布:与智能任务节点能力一致,模型可读文件 / 联网 / 执行命令。直接描述你要完成的任务即可。": "This is an <b>agent session</b> canvas: same capabilities as an agent-task node; the model can read files / network / run commands. Just describe the task you want done.",
    ...
    "可询问当前工作流状态，或让助手修改画布（每次修改会弹窗确认）": "Ask about the workflow, or have the assistant edit the canvas (each edit asks for confirm)",
    ...
    "可查看全部画布列表；改节点图或删除工作流会弹窗确认":
      "Can list all canvases; graph edits or deleting a workflow ask for confirm",
    "我能看到当前工作流、节点与配置，也可参考其他画布列表。\n可以说「总结画布」或「搭一个 xxx 工作流」。\n改节点图或删除工作流前会请你确认。":
      "I can see the workflow, nodes, and settings, and can list other canvases.\nTry \"summarize the canvas\" or \"build a xxx workflow\".\nGraph edits or deleting a workflow will ask for confirmation.",
    "工作流名称不唯一，请改用 id：": "Workflow name is not unique, use id: ",
    "找不到工作流：": "Workflow not found: ",
    ...
    "已删除工作流：": "Deleted workflow: ",
    "将删除该工作流及其全部本地数据文件（含节点图像资产）。此操作不可恢复。": "This will delete the workflow and all of its local data files (including node image assets). This cannot be undone.",
