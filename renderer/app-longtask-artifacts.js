 ...
 *
 * 需求：长任务在每个环节完成时**清点本次的产物**，把每一件适用放置在画布上的产物
 * **逐个摆出来**并排好版 —— 用户不用去翻条带或文件夹，画布上直接看到这次跑出了什么
 * （图片出缩略图、音视频可播、文本可读摘要、其它文件给路径 + 打开入口）。
 ...
 *   ① 清点：从「本环节自己的状态命名空间」里取此刻真的读得到的绝对路径（父命名空间里
 *      继承来的上游路径不算本环节产物 —— 否则每个下游环节都会把上游的图再摆一遍）；
...
function ltArtOwnPaths(run, path, files) {
  ...
  /* 父空间（继承来的输入）里出现过的路径剔除 */
  const inherited = new Set();
  if (typeof ltStateFlat === "function" && typeof ltPathParent === "function") {
    const parentFlat = ltStateFlat(run, ltPathParent(String(path || "")));
...
/* 落画布主入口（引擎在「刚写完的那一刻」调）：清点 → 逐个放置 / 更新 → 排版。
 * files 可给路径字符串数组或 [{ path }]；回 { ok, placed, updated }。 */
async function ltArtPublish(run, path, files) {
  const out = { ok: false, placed: 0, updated: 0 };
  ...
  if (!wf) return out;
  const own = ltArtOwnPaths(run, path, files).slice(0, LT_ART_MAX);
