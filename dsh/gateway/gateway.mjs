...
function applySettings(dshHome, effort, mtnodeProviders, permissionPreset) {
  ...
  /* mtnode 旧档位兼容:none/无 → off */
  const raw = String(effort || 'high').toLowerCase()
  const eff = raw === 'none' || raw === '无' ? 'off' : EFFORTS.includes(raw) ? raw : 'high'
