...
/** Durable session events, or [] when the host has not materialized the log yet. */
...
export function sessionMode(session) {
  const userMsg = sessionEvents(session).find((e) => e.type === 'user/message')
  return classifyTask(extractText(userMsg?.data))
}

const SHELL_CANDIDATES = ['pwsh', 'bash', 'zsh', 'sh', 'shell']
const CORE_ALIASES = {
  read: ['read', 'Read', 'read_file'],
  write: ['write', 'Write', 'write_file'],
  edit: ['edit', 'Edit', 'str_replace_editor'],
  glob: ['glob', 'Glob'],
  grep: ['grep', 'Grep'],
...
export function pickShell(available) {
...
export function resolveCoreNames(wanted, available) {
  const out = []
  for (const name of wanted) {
    const aliases = CORE_ALIASES[name] || [name]
...
/**
 * First-turn tool surface for THIS catalog. Missing shells/editors are skipped
 * (never thrown). If no coding core exists, the full catalog is kept.
 */
export function firstTurnTools(tools, wantedNames) {
  const list = Array.isArray(tools) ? tools : []
  const available = new Set(list.map((tool) => tool && tool.name).filter(Boolean))
  const core = new Set(resolveCoreNames(wantedNames || [], available))
  const shell = pickShell(available)
  if (shell) core.add(shell)
  for (const name of available) {
    if (isHostTool(name)) core.add(name)
  }
  const hasCoding = [...core].some((name) => !isHostTool(name) || name === 'run_code')
  if (!hasCoding) return list
  return list.filter((tool) => core.has(tool.name))
}

/** Best-effort first-turn task text: stored capture, durable log, then inbox. */
export function liveTaskText(agent, session, stored) {
  if (stored) return stored
  const fromEvents = extractText(sessionEvents(session).find((e) => e.type === 'user/message')?.data)
...
/**
 * Append routing guidance. Do NOT strip `deployment:persona` — MTNode keeps
 * canvas / host instructions there; `/persona/i` would wipe them.
 */
export function applyPersona(sections, personaText) {
  const rest = (sections || []).filter(
    (section) => section.name !== 'router-persona' && section.name !== 'persona',
  )
