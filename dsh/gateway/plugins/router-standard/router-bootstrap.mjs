...
import {
  ...
  isComplexTask, extractText, classifyTask, sessionEvents, firstTurnTools, liveTaskText,
...
export function apply(ctx, _config) {
  ...
  function modeOf(session, agent) {
    if (!session) return 'weak'
    const override = overrides.get(session.id)
    if (override !== undefined) return override
    const liveAgent = agent || [...agents.values()].find((a) => a.session === session)
    const pending = liveTaskText(liveAgent, session, firstUserText.get(session.id))
    if (pending) return classifyTask(pending)
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      const mode = modeOf(session, agent)
      const persona = personaFor(mode, agent.options?.model)

      // Keep host deployment:persona (canvas / MTNode). Only append routing text.
      const sections = applyPersona(assembled.sections, persona)

      // After the first durable tool call, keep the full catalog (persona only).
      // Never blank contexts: MTNode injects canvas/vision guidance there.
      ...
      return {
        ...assembled,
        sections,
        tools: firstTurnTools(assembled.tools, coreFor(mode)),
      }
    } catch {
      // Assemble throws kill the JSON-RPC child ("runtime is not running").
      return assembled
    }
  })
  ...
  ctx.on('session/event', (session, event) => {
    try {
      ...
      if (session && !firstUserText.has(session.id) && text.trim()) {
    ...
    } catch { /* duplicate/ordering races or host event-shape mismatch: skip */ }
  })
  ...
  const registerTool = (tool) => {
    ...
    } catch { /* host register() contract mismatch must not fail plugin load */ }
  }
