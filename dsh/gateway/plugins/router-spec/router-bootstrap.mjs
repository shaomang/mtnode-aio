...
export function apply(ctx, config) {
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    try {
      ...
      const persona = routerMode === 'standard'
        ? RL_PERSONA
        : personaFor(mode, agent.options?.model)
      const sections = applyPersona(assembled.sections, persona)
      const wanted = routerMode === 'standard'
        ? ['str_replace_editor', 'read', 'edit', 'write']
        : legacyCore(mode)
      ...
      return {
        ...assembled,
        sections,
        tools: firstTurnTools(assembled.tools, wanted),
      }
    } catch {
      return assembled
    }
