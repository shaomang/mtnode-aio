...
const GET_DESC =
  NODE_LOCK +
  'Read the CURRENT MTNode canvas PLUS app context: workflow name, every VISIBLE node in the current task/super scope (id, kind, title, position, tags, prompt/text/task/goal/steps/parentTaskId/parentSuperId/note/expandW/expandH/savePath/waitPath/waitIntervalSec, timerMode/timerAt/timerEverySec/timerCron/timerArmed/timerNextAt, providerId/provider/model, globalRefs, size for proc_image, ctrlAction/ctrlRole for control, judgeResult; db/dbCount for DATABASE super nodes, dev/devPath/devStatus/devKind/devColor/devModel/devProvider for DEV super nodes, dbNodeId/dbName/compiledAt for db_replica, execPath/execIcon/execColor for execute nodes), taskFocus, superFocus, taskTree, superTree (all super nodes), tagCatalog, marks, wires (each: from/to titles, and for UML-style relationship wires rel:true + relLabel + relArrow), groups, camera, UI view, imageSizes, markColors, and workflows. Node body fields (input_text.text, prompt, task, goal) default to FULL text (with bodyLimit they are truncated); *Len fields report character counts only.\n\n' +
...
const NODE_SPEC = {
  ...
  properties: {
    ...
    devColor: {
      type: 'string',
      description:
        'dev super node: custom frame + running-glow color as #rrggbb (empty = element-type default). The user changes it with the small HSV swatch button in the node header.',
    },
    devModel: {
      type: 'string',
      description:
...
const UPDATE_SPEC = {
  ...
  properties: {
    ...
    devColor: { type: 'string', description: 'dev super node: custom frame color #rrggbb, or empty for the element-type default.' },
    devModel: {
      type: 'string',
      description:
