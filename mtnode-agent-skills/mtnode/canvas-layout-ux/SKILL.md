...
1. 排版是**自动**的：任何改结构的编辑（建图 / 连线 / 删除 / 标注 / 改尺寸 / 分组）都会自动跑一次，不需要你调用任何「排版」动作
2. 算法保证：列顺序 = 层顺序并按行折起，每层包围盒收进 16:9~9:16（结构上收不进会在回执 warnings 里提示「仍是长条」）；同层里上游在左或在上；节点互不重叠
3. 用户手动「一键排版」按钮走同一套算法；**不要**替用户手摆 x/y 来「整理画面」
4. 真要微调（例如只挪一两个节点）：传 `layout:false` 再用 `update` / `updateMarks` 改坐标；勿增删节点、勿改连线
