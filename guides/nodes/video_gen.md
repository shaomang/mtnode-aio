...
节点默认走内置 H3 链（首末帧 / 多参考）。想跑**自己搭的 ComfyUI 图**：展开节点参数面板 → 「工作流来源」选 **自建 ComfyUI 工作流**。
...
- **端子**：图里被「提升为节点参数」的字段会在节点上各占一个端子——**端口 1 = 文本 · 端口 2+ = 素材（图 / 视频 / 音频）**，顺序就是参数表顺序，可用 ▲▼ 调整。默认参数表取扫描建议（提示词 / LoadImage / 种子…），可增删改名。
- **直填值**：每个参数都可在面板上直接填值；**端子接了数据时端子优先**，端子没接才用直填值，都没有就沿用工作流 JSON 里的原值。素材填本机绝对路径，生成时自动上传到 ComfyUI。
...
- 自建模式下 **时长 / 分辨率 / 采样 / 4K 超分补帧等内置参数一律失效**（由工作流图自己决定），也不做 24G 钳制；抽卡、进度、取消、输出路径与全局互斥照常。
