...
When a smart run fails (most often a provider 429 rate limit, a 5xx, or a network blip) the item is no longer skipped: the app **waits 5 seconds and resends the same round**, up to 5 times, and only then reports failure. While it waits the round still counts as "running" — the plan does not advance, messages you type still queue up, and the run queue in the corner keeps showing that session. A resend **prefers to continue from where it stopped** (same engine session, keeping the text already produced, so no tokens are burned twice); only when it cannot resume does it re-run the whole round, and then the leftover text from the failed round is cleared first so nothing doubles up.

**Pressing ■ stops immediately and never enters the resend ladder.** Sessions, smart task nodes, the global assistant, run-queue rows, the "later / abort" buttons on question cards, and "stop all" all share one termination verdict: once a round is marked dead the remaining retry budget is void on the spot — it does not wait a second, it does not re-run once, and the UI settles into (stopped) right away. The verdict is per run key, so stopping one session never cancels another parallel session's resend, and a new round you start after stopping is not killed by the previous round's stop. A run auto-terminated after a long silent stretch is not re-burned either (that would be holding the UI hostage, not recovering).
