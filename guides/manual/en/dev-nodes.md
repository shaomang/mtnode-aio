...
- **开发 (Develop)**: shows the module title / overview / status / **Agent model**; you type what to build or iterate, then it runs in a **new bound session** (workspace = project root, titled 开发 · 模块名, history kept — and it runs with the block's own or inherited model, preset **and** thinking effort).
  - **The bound session doesn't read the canvas — it writes back by id**: that session has **no** `mtnode_canvas_get` / `mtnode_app` this round (the host doesn't register them; a call fails) — a Develop run edits real files under the project root, so a whole-canvas snapshot carries no information for it, and the canvas state is whatever the **dev task brief** states. The brief carries **this node's id**, so at the end it patches exactly **one** object — `mtnode_canvas_edit` `update` by that id, writing back **overview (note) · status (devStatus) · core file list (devFiles)** — **without reading the graph first just to discover its own id** (that read is re-sent on every step). `mtnode_canvas_edit` is therefore the only canvas tool the session keeps, and it touches **only this node**; if you want the AI to change other nodes too, do it in an **agent session** and make sure that session's **Canvas-free** switch is off — that switch drops the canvas edit tool as well, see [Agent task & session](#agent-nodes). 细化 (Refine) uses the same brief and the same rules.
...
| --- | --- | --- |
| **Assistant** (the ✦ panel right of the canvas) | typed-in directory > **canvas project root** > canvas working directory (toolbar) > app default directory | While the scope is **current canvas only**, the typed directory is **ignored** (the field is read-only; it always follows the canvas on your screen, project root first) — switch the scope to **global** to pin a folder. Each run freezes **both its canvas and its directory** at start; switching canvases mid-run changes nothing. |
| **Agent session** (toolbar “Agent sessions”) | the session's typed / 📂 picked directory > **project root of the canvas it belongs to** > app default directory | The canvas working directory is **not** a step here: a session **adopts its own canvas the moment it is created** (the one you are looking at then), and the project root resolves against **that** canvas — not the one you switched to — so you can go work on another canvas mid-run without the session bleeding into it. A session with no manual pick follows the project root, the session list groups by that effective value, and the ▣ under each title shows which canvas it owns. |
| **Feature block 开发 / 细化 (Develop / Refine)** | the project root resolved for that block > generic canvas resolution | Each confirmation **creates a new bound session** (**the canvas that block lives on becomes the session's own canvas**) whose workspace is written as **the project root** at creation time (so it behaves like a manual pick: editing the top-level devPath later does not rewrite past sessions — change the directory inside that session instead). If the block resolves no project root, it falls back to the generic order (another dev block's root → canvas folder → default), and the dialog warns you. |
...
The shell's port strip **no longer prints the “Input / Output” captions** — the param name on each port says which way data goes. Tool / function port names are shortened by width at rest, and **expand to the complete name as soon as you hover over or select the node**.
