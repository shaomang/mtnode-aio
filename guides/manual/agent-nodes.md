...
> 一句话目标：把会话的几组开关调到位、用上它的辅助功能，并绕开几个常见误区。

![会话从哪来与几组开关](img/mtnode-agent-02-ui.svg)

*图：会话的三个入口与几组开关——模型提供商、模型、预设档、思考强度、工具许可*

## 几组开关（模型提供商 · 模型 · 预设档 · 思考强度 · 工具许可）

输入区那枚 **⚙ 芯片**点开就是选型菜单，从上到下四格：**模型提供商 → 预设 → 模型 → 思考强度**。

- **模型提供商**：这轮会话走哪一家（DeepSeek 官方或你在「设置 → 模型服务」里加的服务商）。
  先在这里定家，下一格「模型」就只列这一家的模型；换一家会把模型一起拨成该家的第一只，
  不会留下「A 家的模型配 B 家」这种只在运行时才炸的组合。
- **模型**：flash 级够绝大多数活；长文推理、复杂抽取、写代码换更强的。只列当前提供商的模型。
